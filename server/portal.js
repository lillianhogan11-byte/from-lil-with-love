...
module.exports = function registerPortalRoutes(app, db) {
  ...
  app.post('/portal/api/custom-orders', authMiddleware, (req, res) => {
    ...
    if (!customer_name || !description) return res.status(400).json({ error: 'Name and description required' });
  ...
  app.post('/portal/api/mileage', authMiddleware, (req, res) => {
    ...
    if (!date || !miles) return res.status(400).json({ error: 'Date and miles required' });
