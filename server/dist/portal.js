...
exports.default = registerPortalRoutes;
...
function registerPortalRoutes(app, db) {
    // ── Google OAuth ───────────────────────────────────────
    ...
    // ── Dashboard ─────────────────────────────────────────
    app.get('/portal/api/dashboard', authMiddleware, (req, res) => {
        const now = new Date();
    ...
    // ── Orders ────────────────────────────────────────────
    app.get('/portal/api/orders', authMiddleware, (req, res) => {
        const orders = db.prepare('SELECT * FROM orders ORDER BY created_at DESC').all();
        res.json(orders);
    });
    app.patch('/portal/api/orders/:id', authMiddleware, (req, res) => {
        const { status } = req.body;
        db.prepare('UPDATE orders SET status = ? WHERE id = ?').run(status, req.params.id);
        res.json(db.prepare('SELECT * FROM orders WHERE id = ?').get(req.params.id));
    });
    // ── Custom Orders ─────────────────────────────────────
    app.get('/portal/api/custom-orders', authMiddleware, (req, res) => {
        res.json(db.prepare('SELECT * FROM custom_orders ORDER BY created_at DESC').all());
    });
    app.post('/portal/api/custom-orders', authMiddleware, (req, res) => {
        const { customer_name, customer_phone, customer_email, description, total, deposit, due_date, notes } = req.body;
    ...
    app.patch('/portal/api/custom-orders/:id', authMiddleware, (req, res) => {
        const { status, notes, total, deposit, due_date } = req.body;
    ...
    // ── Income ────────────────────────────────────────────
    app.get('/portal/api/income', authMiddleware, (req, res) => {
        res.json(db.prepare('SELECT * FROM bakery_income ORDER BY date DESC').all());
    });
    app.post('/portal/api/income', authMiddleware, (req, res) => {
        const { date, description, amount, source, notes } = req.body;
        if (!date || !amount) {
            res.status(400).json({ error: 'Date and amount required' });
            return;
        }
    ...
    app.delete('/portal/api/expenses/:id', authMiddleware, (req, res) => {
        db.prepare('DELETE FROM bakery_expenses WHERE id=?').run(req.params.id);
        res.json({ ok: true });
    });
    // ── Inventory ─────────────────────────────────────────
    app.get('/portal/api/inventory', authMiddleware, (req, res) => {
    ...
    app.post('/portal/api/inventory', authMiddleware, (req, res) => {
        const { name, category, unit, quantity, reorder_level, cost_per_unit, supplier_id, notes } = req.body;
        if (!name) {
    ...
    app.patch('/portal/api/inventory/:id', authMiddleware, (req, res) => {
        const { quantity, cost_per_unit, reorder_level, notes } = req.body;
    ...
    app.delete('/portal/api/inventory/:id', authMiddleware, (req, res) => {
        db.prepare('DELETE FROM inventory_items WHERE id=?').run(req.params.id);
        res.json({ ok: true });
    });
    // ── Recipes ───────────────────────────────────────────
    app.get('/portal/api/recipes', authMiddleware, (req, res) => {
        res.json(db.prepare('SELECT * FROM bakery_recipes ORDER BY name').all());
    });
    app.post('/portal/api/recipes', authMiddleware, (req, res) => {
    ...
    app.delete('/portal/api/recipes/:id', authMiddleware, (req, res) => {
        db.prepare('DELETE FROM bakery_recipes WHERE id=?').run(req.params.id);
        res.json({ ok: true });
    });
    // ── Suppliers ─────────────────────────────────────────
    app.get('/portal/api/suppliers', authMiddleware, (req, res) => {
        res.json(db.prepare('SELECT * FROM suppliers ORDER BY name').all());
    });
    app.post('/portal/api/suppliers', authMiddleware, (req, res) => {
    ...
    app.delete('/portal/api/suppliers/:id', authMiddleware, (req, res) => {
        db.prepare('DELETE FROM suppliers WHERE id=?').run(req.params.id);
        res.json({ ok: true });
    });
    // ── Events ────────────────────────────────────────────
    app.get('/portal/api/events', authMiddleware, (req, res) => {
        res.json(db.prepare('SELECT * FROM events ORDER BY date DESC').all());
    });
    app.post('/portal/api/events', authMiddleware, (req, res) => {
    ...
    app.delete('/portal/api/events/:id', authMiddleware, (req, res) => {
        db.prepare('DELETE FROM events WHERE id=?').run(req.params.id);
        res.json({ ok: true });
    });
    // ── Mileage ───────────────────────────────────────────
    app.get('/portal/api/mileage', authMiddleware, (req, res) => {
        res.json(db.prepare('SELECT * FROM mileage_log ORDER BY date DESC').all());
    });
    app.post('/portal/api/mileage', authMiddleware, (req, res) => {
        const { date, origin, destination, miles, purpose } = req.body;
        if (!date || !miles) {
            res.status(400).json({ error: 'Date and miles required' });
            return;
        }
    ...
    app.delete('/portal/api/mileage/:id', authMiddleware, (req, res) => {
        db.prepare('DELETE FROM mileage_log WHERE id=?').run(req.params.id);
        res.json({ ok: true });
    });
    // ── Tax Export ────────────────────────────────────────
    app.get('/portal/api/taxes/export', authMiddleware, (req, res) => {
        const year = req.query.year || String(new Date().getFullYear());
        const income = db.prepare('SELECT * FROM bakery_income WHERE date LIKE ? ORDER BY date').all(`${year}%`);
        const expenses = db.prepare('SELECT * FROM bakery_expenses WHERE date LIKE ? ORDER BY date').all(`${year}%`);
        const miles = db.prepare('SELECT * FROM mileage_log WHERE date LIKE ? ORDER BY date').all(`${year}%`);
    ...
    // ── Menu Management ───────────────────────────────────
    app.get('/portal/api/menu-items', authMiddleware, (req, res) => {
        res.json(db.prepare('SELECT * FROM menu_items ORDER BY category, name').all());
    });
    app.patch('/portal/api/menu-items/:id', authMiddleware, (req, res) => {
    ...
    app.post('/portal/api/menu-items', authMiddleware, (req, res) => {
    ...
    app.post('/api/pos/orders', authMiddleware, (req, res) => {
        ...
        try {
            ...
            const req2 = http_1.default.request({
                hostname: 'localhost',
                port: 18789,
                path: '/api/notify',
                method: 'POST',
